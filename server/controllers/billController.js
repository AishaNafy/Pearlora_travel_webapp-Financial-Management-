...
exports.getUserBills = async (req, res) => {
  try {
    const bills = await Bill.find({ userId: req.params.userId }).populate('userId', 'name email');
    res.status(200).json({ success: true, data: bills });
  } catch (error) {
    res.status(500).json({ success: false, error: 'Server Error' });
  }
...
exports.getBillsByCategory = async (req, res) => {
  try {
    const bills = await Bill.find({ category: req.params.category }).populate('userId', 'name email');
    res.status(200).json({ success: true, data: bills });
  } catch (error) {
    res.status(500).json({ success: false, error: 'Server Error' });
  }
